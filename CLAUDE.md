...
Web application for the Entity_Starter project.

**Package**: `@sudobility/entityentitystarter_app` (private, BUSL-1.1)
...
- **entityentitystarter_types** — Shared type definitions; imported transitively via entityentitystarter_client
- **entityentitystarter_client** — API client SDK with TanStack Query hooks; provides data fetching layer
- **entityentitystarter_lib** — Business logic library with `useHistoriesManager` hook; primary integration point for this app
- **entityentitystarter_api** — Backend server that this app communicates with (both default to port 8022)
- **entityentityentitystarter_app_rn** — React Native counterpart of this web app; shares entityentitystarter_client, entityentitystarter_lib, and entityentitystarter_types
...
- API URL: `.env` defaults to `localhost:8022` to match the API server (`entityentitystarter_api`) -- verify `VITE_API_URL` matches your running API if using a different port
