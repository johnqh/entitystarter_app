...
# clone_starter.sh - Clone all 6 entitystarter projects from GitHub to a new app
#
# Usage:
#   ./clone_starter.sh
#   ./clone_starter.sh "Cravings"
#
# This script clones the 6 entitystarter repos from GitHub (latest main branch),
...
confirm() {
    ...
    echo "  Source repos:"
    for suffix in "${SUFFIXES[@]}"; do
        echo "    git@github.com:${GITHUB_ORG}/entitystarter_${suffix}.git"
    done
    echo ""
...
clone_project() {
    local suffix="$1"
    local repo="git@github.com:${GITHUB_ORG}/entitystarter_${suffix}.git"
    ...
    echo "📥 Cloning entitystarter_${suffix} → ${APP_LOWER}_${suffix}"
...
process_project() {
    ...
    if [ -f "$dst/package.json" ]; then
        # Rename package name
        rename_in_file "$dst/package.json" "@sudobility/entitystarter_${suffix}" "@sudobility/${APP_LOWER}_${suffix}"
        rename_in_file "$dst/package.json" "\"entitystarter_${suffix}\"" "\"${APP_LOWER}_${suffix}\""

        # Rename all entitystarter dependencies
        for dep_suffix in "${SUFFIXES[@]}"; do
            rename_in_file "$dst/package.json" "@sudobility/entitystarter_${dep_suffix}" "@sudobility/${APP_LOWER}_${dep_suffix}"
        done

        # Rename keywords
        rename_in_file "$dst/package.json" "\"entitystarter\"" "\"${APP_LOWER}\""
    fi

    # --- Source code imports ---
    for dep_suffix in "${SUFFIXES[@]}"; do
        rename_in_all_sources "$dst" "@sudobility/entitystarter_${dep_suffix}" "@sudobility/${APP_LOWER}_${dep_suffix}"
    done
    ...
    if [ -f "$dst/CLAUDE.md" ]; then
        ...
        for dep_suffix in "${SUFFIXES[@]}"; do
            rename_in_file "$dst/CLAUDE.md" "entitystarter_${dep_suffix}" "${APP_LOWER}_${dep_suffix}"
        done
        # Replace display name references (Entity_Starter → AppName)
        rename_in_file "$dst/CLAUDE.md" "Entity_Starter" "$APP_NAME"
        rename_in_file "$dst/CLAUDE.md" "entitystarter" "$APP_LOWER"
    fi
}
...
process_api_specific() {
    ...
    if [ -f "$dst/src/db/schema.ts" ]; then
        rename_in_file "$dst/src/db/schema.ts" "pgSchema(\"entitystarter\")" "pgSchema(\"${APP_LOWER}\")"
        rename_in_file "$dst/src/db/schema.ts" "entitystarter_histories" "${APP_LOWER}_histories"
        rename_in_file "$dst/src/db/schema.ts" "\"entitystarter\"" "\"${APP_LOWER}\""
    fi

    # Rename schema name in test
    if [ -f "$dst/src/db/schema.test.ts" ]; then
        rename_in_file "$dst/src/db/schema.test.ts" "\"entitystarter\"" "\"${APP_LOWER}\""
        rename_in_file "$dst/src/db/schema.test.ts" "'entitystarter'" "'${APP_LOWER}'"
    fi

    # Rename schema name in db/index.ts
    if [ -f "$dst/src/db/index.ts" ]; then
        rename_in_file "$dst/src/db/index.ts" "entitystarter" "${APP_LOWER}"
    fi

    # Rename env var references in .env.example
    if [ -f "$dst/.env.example" ]; then
        rename_in_file "$dst/.env.example" "Entity_Starter API" "${APP_NAME} API"
        rename_in_file "$dst/.env.example" "entitystarter" "${APP_LOWER}"
    fi
...
process_app_specific() {
    ...
    if [ -f "$dst/scripts/push_all.sh" ]; then
        for dep_suffix in "${SUFFIXES[@]}"; do
            rename_in_file "$dst/scripts/push_all.sh" "entitystarter_${dep_suffix}" "${APP_LOWER}_${dep_suffix}"
        done
    fi

    # Update clone_starter.sh
    if [ -f "$dst/scripts/clone_starter.sh" ]; then
        rename_in_file "$dst/scripts/clone_starter.sh" "entitystarter" "${APP_LOWER}"
        rename_in_file "$dst/scripts/clone_starter.sh" "Entity_Starter" "${APP_NAME}"
    fi

    # Update localStorage key in WorkspacesPage
    if [ -f "$dst/src/pages/WorkspacesPage.tsx" ]; then
        rename_in_file "$dst/src/pages/WorkspacesPage.tsx" "entitystarter_last_entity" "${APP_LOWER}_last_entity"
    fi
...
process_app_rn_specific() {
    ...
    if [ -f "$dst/app.json" ]; then
        rename_in_file "$dst/app.json" "Entity_Starter" "${APP_NAME}"
        rename_in_file "$dst/app.json" "entitystarter-app" "${APP_LOWER}-app"
        rename_in_file "$dst/app.json" "com.sudobility.entitystarter" "com.sudobility.${APP_LOWER}"
    fi
}
...
init_git_repo() {
    ...
    (
        ...
        git init -q
        git add -A
        git commit -q -m "Initial commit: cloned from entitystarter"
    )
}
